from intcode_computer import IntcodeComputer
from itertools import permutations

def test_1():
    intcode_list = [3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0]

    phase_setting = (4,3,2,1,0)
    computers = [IntcodeComputer(intcode_list) for i in range(5)]
    current_value = 0
    for phase, computer in zip(phase_setting, computers):
        current_value = next(computer.execute(phase, current_value))

    assert current_value == 43210

def test_2():
    intcode_list = [3,23,3,24,1002,24,10,24,1002,23,-1,23,
                    101,5,23,23,1,24,23,23,4,23,99,0,0]

    phase_setting = (0,1,2,3,4)
    computers = [IntcodeComputer(intcode_list) for i in range(5)]
    current_value = 0
    for phase, computer in zip(phase_setting, computers):
        current_value = next(computer.execute(phase, current_value))

    assert current_value == 54321

def test_3():
    intcode_list = [3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,
                    1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0]

    phase_setting = (1,0,4,3,2)
    computers = [IntcodeComputer(intcode_list) for i in range(5)]
    current_value = 0
    for phase, computer in zip(phase_setting, computers):
        current_value = next(computer.execute(phase, current_value))

    assert current_value == 65210

def test_part_1():
    intcode_list = list(map(int, open('input.txt').read().split(',')))
    
    best_value = 0
    best_phase_setting = None
    for phase_setting in permutations(range(5)):
        computers = [IntcodeComputer(intcode_list) for i in range(5)]
        current_value = 0
        for phase, computer in zip(phase_setting, computers):
            current_value = next(computer.execute(phase, current_value))
        if current_value > best_value:
            best_value = current_value
            best_phase_setting = phase_setting

    assert best_value == 880726

def test_part_2_example_1():
    intcode_list = [3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,
                    27,4,27,1001,28,-1,28,1005,28,6,99,0,0,5]
    
    final_value = 0
    phase_setting = (9,8,7,6,5)
    computers = [IntcodeComputer(intcode_list) for i in range(5)]

    for phase, computer in zip(phase_setting, computers):
        computer.input_list.append(phase)

    current_value = 0
    while computers[-1].halt_flag != True:
        for i, computer in enumerate(computers):
            try:
                for output_value in computer.execute(current_value):
                    print(output_value)
                    current_value = output_value
            except:
                pass
        final_value = current_value
    assert final_value == 139629729

def test_part_2_example_2():
    intcode_list = [3,52,1001,52,-5,52,3,53,1,52,56,54,1007,54,5,55,1005,55,26,1001,54,
                    -5,54,1105,1,12,1,53,54,53,1008,54,0,55,1001,55,1,55,2,53,55,53,4,
                    53,1001,56,-1,56,1005,56,6,99,0,0,0,0,10]
    
    final_value = 0
    phase_setting = (9,7,8,5,6)
    computers = [IntcodeComputer(intcode_list) for i in range(5)]

    for phase, computer in zip(phase_setting, computers):
        computer.input_list.append(phase)

    current_value = 0
    while computers[-1].halt_flag != True:
        for i, computer in enumerate(computers):
            try:
                for output_value in computer.execute(current_value):
                    print(output_value)
                    current_value = output_value
            except:
                pass
        final_value = current_value
    assert final_value == 18216

def test_part_2_puzzle():
    intcode_list = list(map(int, open('input.txt').read().split(',')))

    best_value = 0
    best_phase_setting = None
    for phase_setting in permutations(range(5, 10)):
        computers = [IntcodeComputer(intcode_list) for i in range(5)]

        for phase, computer in zip(phase_setting, computers):
            computer.input_list.append(phase)

        current_value = 0
        while computers[-1].halt_flag != True:
            for i, computer in enumerate(computers):
                try:
                    for output_value in computer.execute(current_value):
                        current_value = output_value
                except IndexError:
                    pass
        final_output = current_value
        if final_output > best_value:
            best_value = final_output
            best_phase_setting = phase_setting

    assert best_value == 4931744
