...
def parse_opcode(opcode):
    ...
    immediate_mode_mask = [0 for i in range(opcode_n_params[op_segment] - len(str(opcode)[:-2]) - 1)]
    for digit in reversed(str(opcode)[:-2]):
    ...
    if op_segment in storage_ops:
        return op_segment, immediate_mode_mask + [1]
...
# resolve_concrete_values([2, 2, 0], parse_opcode(1102)[1])
#intcode_list[:16]
...
# So the output doesn't obey the immediacy mask. Others probably don't either then.
